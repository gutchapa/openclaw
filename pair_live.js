const { AndroidRemote } = require('androidtv-remote');
const host = '192.168.1.11'; // Targeting the Living Room TV first based on probability
const options = {
    pairing_port: 6467,
    remote_port: 6466,
    name: 'OpenClaw API',
    cert: {} // Using ephemeral cert for this run
};

const remote = new AndroidRemote(host, options);

remote.on('secret', () => {
    console.log('[[WAITING_FOR_PIN]] The TV should be showing the 6-digit code right now.');
    // Keep the process alive to hold the socket open for the user to provide the PIN
});

remote.on('error', (err) => {
    console.log('Error:', err);
});

console.log('Sending pairing request to', host, '...');
remote.start().catch(e => console.log("Failed to start", e));

// Keep it alive for 60 seconds
setTimeout(() => {
    console.log('Timeout expired.');
    process.exit(0);
}, 60000);
